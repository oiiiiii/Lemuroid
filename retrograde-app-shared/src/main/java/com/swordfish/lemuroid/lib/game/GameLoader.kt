    ...
    fun load(
        ...
        flow {
            try {
                ...
                val coreLibrary =
                    runCatching {
                        println("Attempting to find core: ${systemCoreConfig.coreID.coreName}, libretroFileName: ${systemCoreConfig.coreID.libretroFileName}")
                        val coreFile = findLibrary(appContext, systemCoreConfig.coreID)
                        if (coreFile == null) {
                            println("Core file not found for: ${systemCoreConfig.coreID.coreName}")
                            throw GameLoaderException(GameLoaderError.LoadCore)
                        }
                        println("Core file found: ${coreFile.absolutePath}")
                        coreFile.absolutePath
                    }.getOrElse { 
                        println("Error finding core: ${it.message}")
                        throw GameLoaderException(GameLoaderError.LoadCore)
                    }
    ...
    private fun findLibrary(
        ...
        val files =
            sequenceOf(
                ...
                context.filesDir,
                context.cacheDir, // 添加缓存目录到搜索范围
            )

        // 添加日志，显示搜索的目录
        files.forEach { 
            println("Searching for core ${coreID.libretroFileName} in: ${it.absolutePath}")
        }
        ...
        if (foundFile != null) {
            println("Found core file: ${foundFile.absolutePath}")
        } else {
            println("Core file not found: ${coreID.libretroFileName}")
        }
