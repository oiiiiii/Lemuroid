...
fun SettingsScreen(
    ...
    LemuroidSettingsPage(modifier = modifier) {
        CoresSettings(
            state = state,
            onChangeCoresFolder = { viewModel.changeCoresStorageFolder() },
...
@Composable
private fun CoresSettings(
    state: SettingsViewModel.State,
    onChangeCoresFolder: () -> Unit,
    indexingInProgress: Boolean,
    viewModel: SettingsViewModel,
) {
    val context = LocalContext.current
    ...
    var showCoresDialog by remember { mutableStateOf(false) }
    var coresInfo by remember { mutableStateOf<List<SettingsViewModel.CoreInfo>>(emptyList()) }
    
    // 获取当前设备支持的架构
    val deviceArchitectures = remember { Build.SUPPORTED_ABIS.toList() }
    ...
    LemuroidCardSettingsGroup(title = { Text(text = "游戏核心") }) {
        ...
        LemuroidSettingsMenuLink(
            title = { Text(text = "所需游戏核心") },
            subtitle = { Text(text = "查看当前ROM库中游戏需要的核心") },
            onClick = {
                CoroutineScope(Dispatchers.IO).launch {
                    viewModel.getRequiredCoresInfo().collect {
                        coresInfo = it
                        showCoresDialog = true
                    }
                }
            },
            enabled = !indexingInProgress,
    ...
    if (showCoresDialog) {
        AlertDialog(
            onDismissRequest = { showCoresDialog = false },
            title = {
                Column(
                    ...
                    Text(text = "所需游戏核心")
                    Text(
                        text = "本机设备架构: ${deviceArchitectures.joinToString(", ")}",
                        fontSize = 14.sp,
            ...
            text = {
                Column(
                    ...
                    LazyColumn(
                        ...
                        items(coresInfo) {
                            Column(modifier = Modifier.padding(vertical = 8.dp)) {
                                Text(
                                    text = "${context.getString(it.systemName.toInt())}: ${it.coreDisplayName}",
                                    fontWeight = FontWeight.Bold,
                                    modifier = Modifier.padding(bottom = 4.dp)
                                )
                                Text(
                                    text = "核心名称: ${it.coreName}",
                                    modifier = Modifier.padding(bottom = 2.dp)
                                )
                                Text(
                                    text = "动态库文件: ${it.libretroFileName}",
                                    modifier = Modifier.padding(bottom = 2.dp)
                                )
                            }
                        }
                    ...
                    Row(
                        ...
                        TextButton(
                            onClick = {
                                val coresText = coresInfo.joinToString("\n\n") {
                                    "${context.getString(it.systemName.toInt())}: ${it.coreDisplayName}\n" +
                                    "核心名称: ${it.coreName}\n" +
                                    "动态库文件: ${it.libretroFileName}"
                                }
                                clipboardManager.setText(AnnotatedString(coresText))
                                // 显示复制成功的Toast提示
                        ...
                        TextButton(
                            onClick = {
                                // 打开默认浏览器访问指定网址
                                val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://buildbot.libretro.com/nightly/android/latest/"))
